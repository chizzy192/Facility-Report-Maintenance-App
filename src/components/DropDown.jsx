...
function DropDown ({ categories, value, onChange }) {
  ...
  return (
    <Listbox value={selected} onChange={setSelected} >
      <Label className="py-1 px-2 text-sm transition-all duration-300 ease-in-out text-text">Category *</Label>
      <div className="relative w-full">
        <ListboxButton className="grid w-full cursor-default grid-cols-1 py-2 pr-2 pl-3 text-left focus-visible:outline-2 focus-visible:-outline-offset-3 focus-visible:outline-border bg-gray-100 dark:bg-gray-900  rounded-xl focus:rounded-xl px-2 hover:outline-3 outline-border text-text-muted shadow-xs items-center">
          ...
          {categories.map((category, index) => (
            ...
            >
              <div className="flex items-center">
                <span className="ml-3 block truncate font-normal group-data-selected:font-semibold">{category}</span>
              </div>
